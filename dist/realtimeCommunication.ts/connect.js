...
const initializeSocket = (httpServer) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("get_data", async () => {
            try {
                const data = await room_sevice_1.default.getAllRooms();
        ...
        socket.on("join-room", async ({ roomId, userId }) => {
            try {
                ...
                if (room) {
                    await room_sevice_1.default.addParticipant(roomId, userId);
                    const roomParticipants = await room_sevice_1.default.getRoomUsers(roomId);
        ...
        socket.on("exit-participant", async ({ roomId, userId }) => {
            try {
                ...
                if (roomParticipants?.length == 0) {
                    await room_sevice_1.default.deleteRoom(roomId);
